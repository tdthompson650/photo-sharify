...
// Not exported so the action is not invokable in production until uploads are enabled.
async function sharePhoto(_prevState, formData) {
  const photo = {
  ...
  try {
    ...
    if (!saved) {
      return { message: 'A photo with that title already exists.' };
    }
